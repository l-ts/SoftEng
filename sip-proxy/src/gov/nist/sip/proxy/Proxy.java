...
// g07
import gov.nist.sip.proxy.extensions.ProxyHelper;
...
/*******************************************************************************/
		...
		" (an error code has normally been"+
            ...
            if ( request.getMethod().equals(Request.ACK) ) {
            	  // g07
            	  proxyHelper.beginCallTimeCount(request);
	  ...
	  if (request.getMethod().equals(Request.BYE) ) {
		...
		clonedRequest.addHeader(via);
		  ...
		  return;
            ...
            // g07
            if ( registrar.hasRegistration(request) && !proxyHelper.isBlocked(request)) {
               
                // g07            
                request = proxyHelper.forwardRequest(request,addressFactory, headerFactory);               
		...
		("Proxy, processRequest(), the registrar cannot help"+
            ...
            // g07
            if (proxyHelper.isBlocked(request) && registrar.hasRegistration(request)){
